...
public class LevelLoader {
        ...
        for (String layerName : new String[] { "sol", "vide", "puddles" }) {
            ...
            for (int i = 0; i < tiledMapTileLayer.getWidth(); i++) {
                for (int j = 0; j < tiledMapTileLayer.getHeight(); j++) {
                    ...
                    if (cell != null) {
                        Vector2 coords = new Vector2(i * tiledMapTileLayer.getTileWidth(),
                                j * tiledMapTileLayer.getTileHeight());
                        for (PolygonMapObject polygonMapObject : cell.getTile().getObjects()
    ...
    public void addObjects(World world) throws LevelLoadException {
        try (Input input = new Input(new FileInputStream("partie-" + Pluton.getPlayerName() + ".bin"))) {
            if (joueur != null) throw new Exception("");
            ...
            for (String layerName : new String[] { "Zombies", "Bats", "Bosses", "Apples", "Joueur" }) {
                ...
                for (MapObject mapObject : mapLayer.getObjects()) {
                    ...
                    for (PolygonMapObject polygonMapObject : tiledMapTileMapObject.getTile().getObjects()
                            .getByType(PolygonMapObject.class)) {
                    ...
                    globalCentroid.set(globalCentroid.x / arrayCentroid.size(),
                            globalCentroid.y / arrayCentroid.size());
                    if (!this.gameWorld.spriteOffsets.containsKey(mapObject.getName())) {
                        this.gameWorld.spriteOffsets.put(mapObject.getName(), globalCentroid.cpy());
                    }
                    Vector2 coords = globalCentroid.cpy().add(tiledMapTileMapObject.getX(),
                            tiledMapTileMapObject.getY());
                    // A modifier si les objets ont plusieurs fixtures.
                    switch (layerName) {
                        case "Apples":
                            gameWorld.add(new Apple(coords, arrayVerticies.get(0),
                                    (float) mapLayer.getProperties().get("heal")));
                            break;
                        case "Zombies":
                            gameWorld.add(new Zombie(coords, arrayVerticies.get(0),
                                    (float) mapLayer.getProperties().get("life"),
                                    (float) mapLayer.getProperties().get("damage"),
                                    (float) mapLayer.getProperties().get("monnaie")));
                            break;
                        case "Bats":
                            gameWorld.add(new Bat(coords, arrayVerticies.get(0),
                                    (float) mapLayer.getProperties().get("life"),
                                    (float) mapLayer.getProperties().get("damage"),
                                    (float) mapLayer.getProperties().get("monnaie")));
                            break;
                        case "Bosses":
                            switch (mapObject.getName()) {
                                case "ZombieBoss":
                                    gameWorld.addBoss(new Boss(new Zombie(coords, arrayVerticies.get(0),
